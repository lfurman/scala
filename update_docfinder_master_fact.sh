...
JOIN (
	SELECT
		...
		GROUP_CONCAT(
			CONCAT(
				'{\"code\":\"', search_code,'\",\"scoreComponents\":{\"overall\":', overall_score, ',\"median\":', median_score, ',\"background\":', background_score, ',\"experience\":', experience_score, ',\"topPercentile\":', CAST(top_percentile AS UNSIGNED), '}}'
			)
			SEPARATOR ','
		) score_components
	FROM
